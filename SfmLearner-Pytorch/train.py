...
from loss_functions import photometric_reconstruction_loss_ssim,photometric_reconstruction_loss, explainability_loss, smooth_loss, smooth_loss_2, compute_errors
...
parser.add_argument('-f', '--training-output-freq', type=int, help='frequence for outputting dispnet outputs and warped imgs at training for all scales if 0 will not output',
                    metavar='N', default=0)
parser.add_argument('--loss', type=str, choices=['l1', 'ssim'], default='l1',
                    help='photo')
parser.add_argument('-pn', '--pose_net', type=str, choices=['PoseExpNet', 'PoseRNN', 'PoseRNN2', 'PoseSep'], default='PoseExpNet')
parser.add_argument('--mini_reproj', action='store_true')
parser.add_argument('--auto_mask', action='store_true')
parser.add_argument('-pn_en', '--pn_encoder', type=str, choices=['conv', 'resnet'], default='conv')
parser.add_argument('-disp', '--disp_mode', type=str, choices=['conv', 'resnet'], default='conv')
parser.add_argument('-encoder', '--encoder_mode', type=str, choices=['share', 'seperate','raw'], default='raw')
# parser.add_argument('-pose', '--pose_mode', type=str, choices=['conv', 'resnet_shared','resnet_sep'], default='conv')
parser.add_argument('--smooth_type', type=str, choices=['smooth1', 'smooth2'], default='smooth1')
best_error = -1
n_iter = 0
...
def main():
    ...
    if args.disp_mode == 'resnet':
        disp_encoder = mono2net.ResnetEncoder(num_layers=18, pretrained=True, num_input_images=1).to(device)
        disp_decoder = mono2net.DepthDecoder(disp_encoder.num_ch_enc).to(device)
        disp_net = mono2net.DispNet_resnet(disp_encoder,disp_decoder).to(device)
        # res_encoder = mono2net.ResnetEncoder(18,True,1).to(device)
        # disp_decoder = mono2net.DepthDecoder(res_encoder.num_ch_enc).to(device)
    elif args.disp_mode == 'conv':
    ...
    if args.pose_net == 'PoseExpNet':
        if args.pn_encoder == 'resnet' and args.encoder_mode == 'share':
            pose_decoder = mono2net.PoseDecoder(disp_encoder.num_ch_enc,args.sequence_length).to(device)
            pose_exp_net = mono2net.PoseNet_resnet(disp_decoder,pose_decoder,args.encoder_mode).to(device)
        elif args.pn_encoder == 'resnet' and args.encoder_mode == 'seperate':
            pose_encoder = mono2net.ResnetEncoder(num_layers=18, pretrained=True, num_input_images = args.sequence_length).to(device)
            pose_decoder = mono2net.PoseDecoder(disp_encoder.num_ch_enc, 1, args.sequence_length - 1).to(device)
            pose_exp_net = mono2net.PoseNet_resnet(pose_encoder,pose_decoder,args.encoder_mode).to(device)
        elif args.pn_encoder == 'conv':
            pose_exp_net = models.PoseExpNet(nb_ref_imgs=args.sequence_length - 1, output_exp=args.mask_loss_weight > 0).to(device)
    elif args.pose_net == 'PoseRNN':
        pose_exp_net = models.PoseRNN(nb_ref_imgs=args.sequence_length - 1, output_exp=args.mask_loss_weight > 0, encoder=args.pn_encoder).to(device)
    elif args.pose_net == 'PoseRNN2':
        pose_exp_net = models.PoseRNN2(nb_ref_imgs=args.sequence_length - 1, output_exp=args.mask_loss_weight > 0, encoder=args.pn_encoder).to(device)
    elif args.pose_net == 'PoseSep':
        pose_exp_net = models.PoseSep(nb_ref_imgs=args.sequence_length - 1, output_exp=args.mask_loss_weight > 0, encoder=args.pn_encoder).to(device)

    if args.pretrained_exp_pose:
        print("=> using pre-trained weights for explainabilty and pose net")
        weights = torch.load(args.pretrained_exp_pose)
        pose_exp_net.load_state_dict(weights['state_dict'], strict=False)
    elif args.encoder_mode is 'raw':
        pose_exp_net.init_weights()

    # 加载模型，模型储存在“./parameters/pretrained.pkl"中
    if args.pretrained_disp:
        print("=> using pre-trained weights for Dispnet")
        #path_name='./parameters/pretrained.pkl'
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(train_loader):
        ...
        if args.loss == 'l1':
            loss_1, warped, diff = photometric_reconstruction_loss(tgt_img, ref_imgs, intrinsics,
                                                                   ...
                                                                   args.rotation_mode, args.padding_mode, args.mini_reproj, args.auto_mask)
        elif args.loss == 'ssim':
            loss_1, warped, diff = photometric_reconstruction_loss_ssim(tgt_img, ref_imgs, intrinsics,
                                                                        depth, explainability_mask, pose,
                                                                        args.rotation_mode, args.padding_mode, args.mini_reproj, args.auto_mask)
        if w2 > 0:
        ...
        if args.smooth_type == 'smooth1':
            loss_3 = smooth_loss(depth)
        elif args.smooth_type == 'smooth2':
            loss_3 = smooth_loss_2(depth, tgt_img)
