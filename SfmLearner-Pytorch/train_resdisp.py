...
from loss_functions import photometric_reconstruction_loss_ssim,photometric_reconstruction_loss, explainability_loss, smooth_loss, compute_errors
...
def main():
    ...
    res_encoder = torch.nn.DataParallel(res_encoder)
    disp_decoder = torch.nn.DataParallel(disp_decoder)
    pose_exp_net = torch.nn.DataParallel(pose_exp_net)

    print('=> setting adam solver')

    optim_params = [
        {'params': res_encoder.parameters(), 'lr': args.lr},
        {'params': disp_decoder.parameters(), 'lr': args.lr},
    ...
    if args.pretrained_disp or args.evaluate:
        ...
        if args.with_gt:
            errors, error_names = validate_with_gt(args, val_loader, res_encoder, disp_decoder, 0, logger, tb_writer)
        else:
            errors, error_names = validate_without_gt(args, val_loader, res_encoder, disp_decoder, pose_exp_net, 0, logger, tb_writer)
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = train(args, train_loader, res_encoder, disp_decoder, pose_exp_net, optimizer, args.epoch_size, logger, tb_writer)
        logger.train_writer.write(' * Avg Loss : {:.3f}'.format(train_loss))

        # evaluate on validation set
        logger.reset_valid_bar()
        if args.with_gt:
            errors, error_names = validate_with_gt(args, val_loader, res_encoder, disp_decoder, epoch, logger, tb_writer)
        else:
            errors, error_names = validate_without_gt(args, val_loader, res_encoder, disp_decoder, pose_exp_net, epoch, logger, tb_writer)
        ...
        save_checkpoint(
            args.save_path, {
                'epoch': epoch + 1,
                'state_dict': res_encoder.module.state_dict()
            },{
                'epoch': epoch + 1,
                'state_dict': disp_decoder.module.state_dict()
            }, {
                'epoch': epoch + 1,
...
def train(args, train_loader, res_encoder, disp_decoder, pose_exp_net, optimizer, epoch_size, logger, tb_writer):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(train_loader):
        ...
        # compute output
        features = res_encoder(tgt_img)
        disparities = disp_decoder(features)
...
@torch.no_grad()
def validate_without_gt(args, val_loader, res_encoder, disp_decoder, pose_exp_net, epoch, logger, tb_writer, sample_nb_to_log=3):
    global device
    batch_time = AverageMeter()
...
@torch.no_grad()
def validate_with_gt(args, val_loader, res_encoder, disp_decoder, epoch, logger, tb_writer, sample_nb_to_log=3):
    global device
    batch_time = AverageMeter()
    ...
    for i, (tgt_img, depth) in enumerate(val_loader):
        ...
        # compute output
        features = res_encoder(tgt_img)
        output_disp = disp_decoder(features)
